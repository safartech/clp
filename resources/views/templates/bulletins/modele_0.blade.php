    ...
    <div class="col s12">
            ...
            @foreach($matieres as $matiere)
                ...
                @if(count($matiere->sousMatieres)== 0)
                    ...
                    if(count($notes)>0){
                        ...
                        <td class="text-center" style="font-size: 10px">{{ $matiere->appreciations[0]->appreciation ?? ""}}</td>
                    </tr>
                @else
                    @php
                    ...
                    @foreach($matiere->sousMatieres as $sousMatiere)
                        ...
                        @if(count($sousMatiere->modules)==0)
                            ...
                            <tr>
                                ...
                                <td class="grey desc text-center"  style="font-size: 10px">{{ $sousMatiere->description }}</td>
                           ...
                           $notes[] = $n;
                            ...
                            $liste_des_moyennes_de_sm [] =  number_format($moy,2) ;
                                ...
                                <td class="text-center">{{ is_numeric($moy)? number_format($moy,2) :"" ?? ""}} </td>
                                <td class="text-center" style="font-size: 10px">{{ $sousMatiere->appreciations[0]->appreciation ?? ""}}</td>
                            </tr>
                        @else
                            @php
                            ...
                            <tr>
                                ...
                                <td class="grey desc text-center" style="font-size: 10px"><b>{{ $sousMatiere->modules[0]->nom }}</b> : {{ $sousMatiere->modules[0]->description }}</td>
                                @php
                                    $moy = null;
                                    $n=0;
                                $pas_de_note = 0;
                           ...
                           $notes[] = $n;
                            ...
                            $liste_de_moyennnes_de_modules[] = number_format($moy,2);
                                ...
                                @endphp
                                <td class="text-center">{{ is_numeric($moy)? number_format($moy,2) :"" ?? ""}}</td>
                                <td class="text-center" style="font-size: 10px">{{ $sousMatiere->modules[0]->appreciations[0]->appreciation ?? ""}}</td>
                            </tr>
                            @foreach($sousMatiere->modules as $i=>$module)
                                @if($i==0) @continue @endif
                                <tr>
                                    <td class="grey desc text-center" style="font-size: 10px"><b>{{ $module->nom }}</b > : {{ $module->description }}</td>
                                    @php
                                        $moy = null;
                                $pas_de_note = 0;
                                $total = 0;
                           ...
                           $notes[] = $n;
                            ...
                            $liste_de_moyennnes_de_modules[] = number_format($moy,2);
                                ...
                                $moy = 0;*/
                                    ...
                                    @endphp
                                    <td class="text-center">{{ is_numeric($moy)? number_format($moy,2) :"" ?? ""}}</td>
                                    <td class="text-center" style="font-size: 10px">{{ $module->appreciations[0]->appreciation ?? ""}}</td>
                                </tr>
                            @endforeach
        ...
        <table class="table table-bordered">
            <tr class="grey">
                ...
                <td class="text-center">{{ count($eleve->absences) }}</td>
            </tr>
            @foreach($conduites as $conduite)
                <tr class="matiere-line"><td class="text-center" colspan="2"><b>{{ $conduite->nom }}</b></td></tr>
                @foreach($conduite->attitudes as $attitude)
                    <tr>
                        <td class="grey"><b>{{ $attitude->nom }}</b></td>
                        <td>{{ $attitude->avis[0]->avis ?? ""}}</td>
                    </tr>
                @endforeach
            @endforeach
        </table>
        ...
        <table class="table table-bordered">
            ...
            <tr style="height: 100px">
                ...
                <td colspan="2"><b>OBSERVATIONS GENERALES: </b> <div style="font-size: 12px">{{ $obs[0]->observation ?? "" }}
                        <br>
                        <br>
                    </div></td>
